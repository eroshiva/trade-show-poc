...
func (ndq *NetworkDeviceQuery) QueryEndpoints() *EndpointQuery {
...
func (ndq *NetworkDeviceQuery) Clone() *NetworkDeviceQuery {
	...
	return &NetworkDeviceQuery{
		...
		withEndpoints: ndq.withEndpoints.Clone(),
...
func (ndq *NetworkDeviceQuery) WithEndpoints(opts ...func(*EndpointQuery)) *NetworkDeviceQuery {
	query := (&EndpointClient{config: ndq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	ndq.withEndpoints = query
	return ndq
}
...
func (ndq *NetworkDeviceQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*NetworkDevice, error) {
	var (
		...
		_spec       = ndq.querySpec()
		loadedTypes = [3]bool{
			ndq.withEndpoints != nil,
			ndq.withSwVersion != nil,
			ndq.withFwVersion != nil,
		}
	)
	if ndq.withSwVersion != nil || ndq.withFwVersion != nil {
		withFKs = true
	}
	if withFKs {
		_spec.Node.Columns = append(_spec.Node.Columns, networkdevice.ForeignKeys...)
	}
	...
	if query := ndq.withEndpoints; query != nil {
		if err := ndq.loadEndpoints(ctx, query, nodes,
			func(n *NetworkDevice) { n.Edges.Endpoints = []*Endpoint{} },
			func(n *NetworkDevice, e *Endpoint) { n.Edges.Endpoints = append(n.Edges.Endpoints, e) }); err != nil {
			return nil, err
		}
	}
	if query := ndq.withSwVersion; query != nil {
		if err := ndq.loadSwVersion(ctx, query, nodes, nil,
			func(n *NetworkDevice, e *Version) { n.Edges.SwVersion = e }); err != nil {
			return nil, err
		}
	}
	if query := ndq.withFwVersion; query != nil {
		if err := ndq.loadFwVersion(ctx, query, nodes, nil,
			func(n *NetworkDevice, e *Version) { n.Edges.FwVersion = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (ndq *NetworkDeviceQuery) loadEndpoints(ctx context.Context, query *EndpointQuery, nodes []*NetworkDevice, init func(*NetworkDevice), assign func(*NetworkDevice, *Endpoint)) error {
	...
	query.Where(predicate.Endpoint(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(networkdevice.EndpointsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.network_device_endpoints
		if fk == nil {
			return fmt.Errorf(`foreign-key "network_device_endpoints" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "network_device_endpoints" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (ndq *NetworkDeviceQuery) loadSwVersion(ctx context.Context, query *VersionQuery, nodes []*NetworkDevice, init func(*NetworkDevice), assign func(*NetworkDevice, *Version)) error {
	...
	nodeids := make(map[string][]*NetworkDevice)
	for i := range nodes {
		if nodes[i].network_device_sw_version == nil {
			continue
		}
		fk := *nodes[i].network_device_sw_version
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "network_device_sw_version" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (ndq *NetworkDeviceQuery) loadFwVersion(ctx context.Context, query *VersionQuery, nodes []*NetworkDevice, init func(*NetworkDevice), assign func(*NetworkDevice, *Version)) error {
	...
	nodeids := make(map[string][]*NetworkDevice)
	for i := range nodes {
		if nodes[i].network_device_fw_version == nil {
			continue
		}
		fk := *nodes[i].network_device_fw_version
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "network_device_fw_version" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
